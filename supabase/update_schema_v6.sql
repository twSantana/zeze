...
-- 1. Adicionar novas colunas na tabela de empreendimentos
ALTER TABLE public.empreendimentos 
  ADD COLUMN IF NOT EXISTS faixa VARCHAR(50),
  ADD COLUMN IF NOT EXISTS drive_url TEXT;

-- 2. Atualizar registros existentes com a classificação automática de faixa
UPDATE public.empreendimentos 
SET faixa = 
  CASE 
    WHEN preco <= 275000 THEN 'Faixa 2'
    WHEN preco > 275000 AND preco <= 400000 THEN 'Faixa 3'
    WHEN preco > 400000 AND preco <= 600000 THEN 'Faixa 4'
    ELSE 'SBPE'
  END;

-- 3. Remover a função get_empreendimentos_bbox antiga
...
-- 4. Recriar a função get_empreendimentos_bbox incluindo as novas colunas
